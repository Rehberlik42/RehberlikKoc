...
import type { SubjectOption } from "@/app/dashboard/teacher/resources/_components/resource-types";
...
function parseAssignments(
  ...
  for (const row of rows) {
    ...
    const kind = (sr as { content_kind?: string | null }).content_kind;
    if (!kind) continue;
...
const STAT_TONES = {
  ...
  sky: "border-sky-600/30 bg-sky-500/10",
  violet: "border-violet-600/30 bg-violet-500/10",
  emerald: "border-emerald-600/30 bg-emerald-500/10",
  amber: "border-amber-600/30 bg-amber-500/10",
  rose: "border-rose-600/30 bg-rose-500/10",
  muted: "border-[var(--border)] bg-[var(--surface-2)]/60",
} as const;
...
function MatrixStatsStrip({ stats }: { stats: MatrixStats }) {
  ...
  return (
    <div className="flex items-stretch gap-2.5 rounded-2xl border border-[var(--border)] bg-[var(--surface)]/60 p-3">
...
function ResourceProgressBars({ items }: { items: ResourceProgress[] }) {
  ...
  return (
    <div className="space-y-2 rounded-2xl border border-[var(--border)] bg-[var(--surface)]/60 p-2.5">
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((res) => (
          <div
            key={res.id}
            className="min-w-[8.5rem] flex-1 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] px-2.5 py-2"
          >
...
const FILTER_OPTIONS: {
...
}[] = [
  {
    ...
    activeCls: "border-rose-600/40 bg-rose-500/15 text-rose-600",
  },
  {
    id: "in_progress",
    label: "Devam Edenler",
    activeCls: "border-amber-600/40 bg-amber-500/15 text-amber-600",
  },
  {
    id: "completed",
    label: "Tamamlananlar",
    activeCls: "border-emerald-600/40 bg-emerald-500/15 text-emerald-600",
  },
  {
    id: "not_started",
        ...
        return (
          <button
            ...
            className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold transition-colors ${
              isActive
                ? opt.activeCls
                : "border-[var(--border)] bg-[var(--surface-2)]/60 text-[var(--text-muted)] hover:text-[var(--text-secondary)]"
            }`}
          >
            {opt.label}
          </button>
...
function TopicDetailPane({
  ...
  return (
    <aside className="rounded-2xl border border-[var(--border)] bg-[var(--surface)]/60 p-3 2xl:sticky 2xl:top-4 2xl:w-72 2xl:shrink-0">
          ...
          {resources.length === 0 || activeResource == null ? (
            ...
            <>
              ...
              <label className="block space-y-1.5">
                <span className={`block ${DETAIL_LABEL_CLS}`}>Notum</span>
                <textarea
                  value={noteValue}
              ...
              <div className="space-y-1">
                <p className={DETAIL_LABEL_CLS}>Koç Notu</p>
                {progress?.coach_note ? (
                  <p className="whitespace-pre-wrap rounded-lg border border-[var(--primary)]/20 bg-[var(--primary)]/5 px-2.5 py-2 text-xs text-[var(--text-secondary)]">
                    {progress.coach_note}
                  </p>
                ) : (
                  <p className="text-xs text-[var(--text-muted)]">
...
function OtherResourcesSection({ items }: { items: MatrixResource[] }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <ul className="divide-y divide-[var(--border)] rounded-xl border border-[var(--border)] bg-[var(--surface)]/60">
        {items.map((res) => (
          <li
            key={res.id}
...
function MatrixLegend() {
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] px-4 py-3 text-[11px] text-[var(--text-muted)]">
...
function SubjectMatrixPanel({
  ...
  const assignableSubjectIds = useMemo(() => {
    ...
    for (const s of subjects) {
      ...
      const examName = (s.examName ?? "").toUpperCase();
  ...
  const handleNoteBlur = () => {
    ...
    if ((current.student_note ?? "") === trimmed) return;

    void persistProgress(
      activeSrtId,
      `note:${activeSrtId}`,
      (prev) => ({ ...prev, student_note: trimmed === "" ? null : trimmed }),
      "Not kaydedilemedi"
    );
  };
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-[var(--border)] bg-[var(--surface)]/60 py-12 text-[var(--text-muted)]">
          <Loader2 className="h-5 w-5 animate-spin text-[var(--accent)]" />
          <p className="mt-2 text-xs">Yükleniyor…</p>
        </div>
      ) : error ? (
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 px-3 py-4 text-center text-xs text-red-500">
          {error}
        </div>
      ) : topicRows.length === 0 ? (
        ...
        <>
          ...
          {resources.length === 0 ? (
            <p className="text-xs text-[var(--text-muted)]">
              Bu derse henüz kaynak atanmadı. Koçun atadığında burada sütun
              olarak görünecek.
            </p>
          ) : null}
          <div className="flex flex-col gap-3 2xl:flex-row 2xl:items-start">
            <div className="min-w-0 flex-1 space-y-3">
              {visibleTopicRows.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface)]/60 px-4 py-10 text-center">
                  <p className="text-xs text-[var(--text-muted)]">
...
export default function StudentResourceMatrix({ studentId, subjects }: Props) {
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-2 text-sm text-[var(--text-muted)]">
          Ders grubu seç; TYT ve AYT matrisleri yan yana görünür. Hücreye
          tıklayarak çalışma durumunu güncelleyebilirsin.
        </p>
      </div>
      ...
      {groupsLoading ? (
        ...
        <div className="rounded-2xl border border-red-500/30 bg-red-500/5 px-4 py-6 text-center text-sm text-red-500">
          {groupsError}
        </div>
      ) : groups.length === 0 ? (
        ...
        <>
          <div className="-mx-1 overflow-x-auto px-1 pb-1">
            <div
              ...
              className="flex w-max min-w-full gap-1 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-1"
            >
              {groups.map((group) => {
            ...
            >
              {hasTyt && selectedGroup.tyt_subject_id != null && (
                <SubjectMatrixPanel
                  ...
                  examLabel="TYT"
                  subjectName={
                    subjectNameById.get(selectedGroup.tyt_subject_id) ?? "Ders"
                  }
                  subjects={subjects}
                />
              )}
              {hasAyt && selectedGroup.ayt_subject_id != null && (
                <SubjectMatrixPanel
                  ...
                  examLabel="AYT"
                  subjectName={
                    subjectNameById.get(selectedGroup.ayt_subject_id) ?? "Ders"
                  }
                  subjects={subjects}
                />
              )}
