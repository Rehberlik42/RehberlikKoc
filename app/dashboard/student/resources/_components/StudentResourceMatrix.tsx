...
function statusChipClass(status: string): string {
  switch (normalizeStatus(status)) {
    case "tamamlandi":
      return "border-emerald-600/50 bg-emerald-500/15 text-emerald-600";
    case "baslandi":
    case "devam_ediyor":
      return "border-amber-600/50 bg-amber-500/15 text-amber-600";
    case "tekrar_gerekli":
      return "border-rose-600/50 bg-rose-500/15 text-rose-600";
    default:
      return "border-[var(--border)] bg-[var(--surface-2)] text-[var(--text-muted)]/50";
  }
}
...
async function fetchSubjectMatrix(
  ...
  const [topicsRes, assignmentsRes] = await Promise.all([
    ...
    supabase
      ...
      .eq("study_resource.subject_id", subjectId)
...
function SubjectMatrixPanel({
  ...
  const loadMatrix = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await fetchSubjectMatrix(studentId, subjectId);
      setData(next);
    } catch (err) {
