  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative w-full max-w-md animate-in fade-in zoom-in-95 rounded-2xl border border-red-500/25 bg-gradient-to-br from-[var(--surface)] to-[var(--bg)] shadow-2xl duration-200"
      >
        <div className="flex items-center justify-between border-b border-red-500/20 px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-bold text-[var(--text-primary)]">Kaynağı Sil?</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex gap-3 border-t border-[var(--border)] px-5 py-4">
          ...
          <button
            ...
            disabled={loading}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-red-600/90 py-2.5 text-sm font-bold text-[var(--text-primary)] transition-colors hover:bg-red-600 disabled:opacity-50"
          >
            {loading ? (
